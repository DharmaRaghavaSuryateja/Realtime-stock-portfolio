...
export const getMyStocks = async (
    ...
    userstocks.map(async (stock) => {
      ...
      if (quote) {
        ...
        const localInvestment =
          (await Convert(stock.purchase_price)
            .from(quote.currency)
            .to(userLocalCurrency)) * stock.quantity;
        const localPresentValue =
          (await Convert(quote.currentMarketPrice)
            .from(quote.currency)
            .to(userLocalCurrency)) * stock.quantity;
  ...
  const sectorSummaries = Object.entries(sectorGroups).map(
    ([sector, stocks]) => {
...
export const createMyStock = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const deleteMyStock = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
