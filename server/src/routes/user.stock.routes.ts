...
router.get(
  '/',
  authenticateToken,
  cacheMiddleware(GET_MY_STOCKS_KEY),
  asyncHandler(getMyStocks),
);

router.post(
  '/',
  authenticateToken,
  validateRequest(createMyStockValidator),
  asyncHandler(createMyStock),
);

router.delete(
  '/:stockId',
  authenticateToken,
  validateRequest(stockIdValidator),
  asyncHandler(deleteMyStock),
);
